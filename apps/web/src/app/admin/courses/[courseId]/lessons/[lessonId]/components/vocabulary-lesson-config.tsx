...
export interface VocabularyLessonConfigRef {
  save: () => Promise<void>;
  hasChanges: () => boolean;
}
...
interface VocabularyLessonConfigProps {
  lessonId: string;
...
export const VocabularyLessonConfig = forwardRef<VocabularyLessonConfigRef, VocabularyLessonConfigProps>(
  function VocabularyLessonConfig({ lessonId, onHasChanges }, ref) {
  ...
  useImperativeHandle(ref, () => ({
    save: async () => {
      ...
      try {
        ...
        await adminApi.upsertLessonVocabulary(lessonId, vocabularyList);
  ...
  }), [lessonId, vocabularyList, hasLocalChanges]);
  ...
  useEffect(() => {
    async function fetchVocabulary() {
      ...
      try {
        const { adminApi } = await import("@/features/admin/api/admin-api");
        const data = (await adminApi.getLessonVocabulary(lessonId)) as VocabularyItem[];
  ...
  const getPartOfSpeechConfig = (pos: string) => {
    return (
      PART_OF_SPEECH_OPTIONS.find((p) => p.value === pos) || PART_OF_SPEECH_OPTIONS[0]
    );
  };
