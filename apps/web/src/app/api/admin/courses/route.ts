...
export async function GET() {
  ...
  try {
    ...
    return apiError(error, "Không thể lấy danh sách khóa học");
  }
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const data = await adminService.createAdminCourse(body);
    return apiOk(data, { status: 201 });
  } catch (error) {
    return apiError(error, "Không thể tạo khóa học");
  }
}
